...
export function applyObjectProps(dst, options, setter) {
...
export function bindObjectProp(src, prop, dst, apply = true, setter) {
    if (apply)
        applyObjectProps(dst, src[prop], setter);
    const r = toRef(src, prop);
    return watch(r, (value) => { applyObjectProps(dst, value, setter); });
}
export function bindObjectProps(src, dst, apply = true, setter) {
    if (apply)
...
export function getMatcapUrl(hash = DEFAULT_MATCAP, format = 1024) {
